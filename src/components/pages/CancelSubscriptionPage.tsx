...
export function CancelSubscriptionPage({
  ...
  // Check if special offer should be shown
  useEffect(() => {
    const hasBeenShown = localStorage.hasSpecialOfferBeenShown();
    if (!hasBeenShown && step === 2) {
      // Show special offer when user reaches step 2 (feedback step)
  ...
  const handleSpecialOfferAccept = () => {
    // Mark offer as shown
    localStorage.markSpecialOfferAsShown();
    setShowSpecialOffer(false);
    
    // Redirect back to manage subscription with success message
    onBack();
    
    // You could also show a success toast here
    console.log('Special offer accepted! User stays with discounted plan.');
  };

  const handleSpecialOfferDecline = () => {
    // Mark offer as shown so it won't appear again
    localStorage.markSpecialOfferAsShown();
    setShowSpecialOffer(false);
  ...
  const handleSpecialOfferClose = () => {
    // Just close the modal without marking as shown
    // This allows the offer to appear again if user tries to cancel later
    setShowSpecialOffer(false);
  };
  ...
  return (
    <>
            ...
            <div className="relative z-10 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-cyan-100">Pro Plan - $299/month</p>
                </div>
              </div>
              <button onClick={onBack} className="p-2 hover:bg-white/20 rounded-full transition-colors">
      ...
      <SpecialOfferModal 
        ...
        onDecline={handleSpecialOfferDecline}
      />
    </>
  );
}